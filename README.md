...
| `hsmdoctor evidence` | Auditor-facing compliance evidence report for policy packs: per-control pass/fail/not-applicable with the objects behind each failure (HTML or JSON) |
...
| `hsmdoctor kmip` | Diagnose a KMIP key-management server over (mutual) TLS: inventory and security posture (custom rules via `--rules`), with SARIF output and `--baseline` CI gating like `scan` (experimental, read-only) |
